...
- `data/almanac/` — 지식 도감(이론 심화 레이어) 콘텐츠. `almanacTypes.ts`에 스키마, `{buildingId}Almanac.ts` 15개에 실제 역사·이론(연표·이미지 출처), `index.ts`가 `Record<BuildingId, BuildingAlmanac>`로 조립하고 잠금 판정 헬퍼(`isAlmanacUnlocked`)도 여기서 export한다. `economicSeasons.ts`는 경제 사계절(봄/여름/가을/겨울) → 자산 5종(주식/부동산/금/원자재/현금) 반응 테이블(`SEASON_REACTIONS`/`REACTION_LABEL`/`REACTION_MULTIPLIER`)을 담은 공유 데이터 모듈이다 — `components/almanac/interactive/EconomicSeasonsWheel.tsx`(도감 위젯)와 `components/minigame/etfLab/PortfolioRoundGame.tsx`(etf-lab 포트폴리오 라운드 미니게임)가 같은 테이블을 참조한다. `components/almanac/`(`KnowledgeCard`/`AlmanacGrid`/`ImageCreditFooter`), `app/almanac/`(허브 + `[id]` 상세), `app/credits/`(이미지 저작자 표시 목록)가 이 데이터를 사용한다. 이미지 파일은 `public/images/almanac/{buildingId}/{imageKey}.jpg`에 위키미디어 커먼즈에서 라이선스를 확인해 다운로드해 두며, 크레딧은 `/credits`에서 확인할 수 있다. `BuildingAlmanac.interactiveWidgetKey`(선택 필드)가 있는 건물은 `KnowledgeCard`에 "직접 만져보며 이해하기" 섹션이 추가로 렌더링된다 — 상세 설계는 `docs/almanac-interactive.md` 참고.
...
- `components/minigame/{building}/`에서 DOM/Pixi/Matter.js 없이 테스트 가능한 계산 로직은 부수효과 없는 순수 함수 파일로 분리하고 co-located `*.test.ts`(Vitest)를 둔다 — 예: `loanCounter/tiltMath.ts`, `seedField/rouletteMath.ts`, `stockStreet/disciplineMath.ts`(FOMO 이벤트 "버틴 나"/"휩쓸렸다면" 배수 계산), `etfLab/portfolioMath.ts`(포트폴리오 라운드 자산 배분·계절 반응 계산). 새 미니게임에 계산 로직이 생기면 이 패턴을 따른다.
