...
// 구역 잠금 해제·던전 3구역처럼 여러 건물을 처음부터 순서대로 깨야만 도달하는 상태를
// 매번 UI로 전부 재생하면 테스트가 지나치게 느리고 취약해진다. 이미 store 단위 테스트
// (store/useGameStore.test.ts)로 검증된 "이전 진행도"는 localStorage에 직접 시드하고,
// 이 테스트가 실제로 검증하려는 마지막 한 걸음(구역 잠금 해제 트리거, 새 미니게임 완주)만
// 실제 UI 플레이로 확인한다.
